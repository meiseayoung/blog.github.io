...
var Map = Class.extend({
	 ...
	 * [初始化地图参数]
	 * @param  {Objec} opts [{
	 *                      	renderTo:"map"    //必填  type:String
	 *                      	center:[]         //可选	  type:Array       默认:[0,0]
	 *                      	minZoom:6		  //可选	  type:Number      默认:0
	 *                      	maxZoom:16		  //可选	  type:Number      默认:16
	 *                      	zoom:8            //可选	  type:Number      默认:8
	 *                      	tilePath: "../"   //可选   type:String     默认：basePath + "pages/volte/mapTiles/tile_baidu_14/"
	 * }]						
	...
	go2Center: function(zoomSize) {
		 ...
		 *@param 动画结束后地图中心点
		 **/
		function _animation(animationName, center, duration) {
	 ...
	 */
	registerMap: function(url, name, style) {
		var me = this;
	 ...
	 * @return {[undefined]}           [undefined]
	 */
	initStyle: function(nameSpace, style) {
		var me = this;
		var features = me.areaInfo[nameSpace].features;
		for (var i = 0; i < features.length; i++) {
			features[i].setStyle(
	 ...
	 * @return {[undefined]}          [undefined]
	 */
	openSelect: function(cbSelect, cbHover) {
		var me = this;
		var map = this.map;
		var position = {};
	 ...
	 * @return {[undefined]}        [undefined]
	 */
	fitBounds: function(nameSpace, areaName,callback) {
		var me = this;
		var map = me.map;
		var extent = null;
		var features = me.areaInfo[nameSpace].features;
	 ...
	 */
	getCenter: function(nameSpace, areaName) {
		var me = this;
		var map = me.map;
		var extent = null;
		var features = me.areaInfo[nameSpace].features;
